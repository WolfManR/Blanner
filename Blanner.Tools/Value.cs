namespace Blanner.Tools;
public class Value<TValue, TKey>(TValue? value) : IEquatable<Value<TValue, TKey>?> {
	public TKey Id => KeySelector is not null ? KeySelector.Invoke(StoredValue) : throw new InvalidOperationException($"Key selector for type {GetType().Name} not set");
